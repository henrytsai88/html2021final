...
def train_SVC(x_train, x_val, y_train, y_val, random_state=0):
    ...
    model = SVC(gamma='auto', degree=2, class_weight='balanced', coef0=4.0, shrinking=True, kernel='rbf', probability=True, random_state=random_state, decision_function_shape='ovo')
...
def train_LogisticRegression(x_train, x_val, y_train, y_val, random_state=0):
    ...
    model = LogisticRegression(class_weight=None, random_state=random_state, solver='lbfgs', penalty='l2', C=5.0, max_iter=1000)
    model.fit(x_train, y_train)
...
def main():
    ...
    model = train_LogisticRegression(x_train, x_val, y_train, y_val, random_state=seed)
